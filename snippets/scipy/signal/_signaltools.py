...
def sosfilt(sos, x, zi=None):
    return spy.signal.sosfilt(sos, x, zi=zi)
    """
...
def sosfiltfilt(sos, x, axis=-1, padtype='odd', padlen=None):
    ...
    sos, n_sections = _validate_sos(sos)  
    #n_sections = sos.shape
    x = np.array(x) #_validate_x(x)
    ...
    (y, zf) = sosfilt(sos, ext, zi=zi* x_0)
    y_0 = axis_slice(y, start=-1, axis=axis)
    (y, zf) = sosfilt(sos, axis_reverse(y, axis=axis), zi=zi * y_0)
...
def decimate(x, q, n=None, ftype='iir', axis=-1, zero_phase=True):
    ...
    #    result_type = np.float
    #b = asarray(b, dtype=result_type)
    #a = asarray(a, dtype=result_type)
